...
impl<T: SegmentTreeSpec> SegmentTree<T> {
    ...
    fn update_upward(&mut self, mut node_idx: usize, segment_size: i64) {
        while node_idx > 1 {
            node_idx /= 2;
            match &self.app[node_idx] {
                Some(f) => {
                    self.val[node_idx] = T::apply(
                        f,
                        &T::op(&self.val[2 * node_idx], &self.val[2 * node_idx + 1]),
                        segment_size,
                    )
                }
                None => {
                    self.val[node_idx] = T::op(&self.val[2 * node_idx], &self.val[2 * node_idx + 1])
                }
            }
        }
    }

    fn update_downward(&mut self, node_idx: usize) {
    ...
    pub fn update(&mut self, mut l: usize, mut r: usize, f: &T::F) {
        ...
        self.update_upward(l0, 1);
        self.update_upward(r0 - 1, 1);
    }
    ...
    pub fn query(&mut self, mut l: usize, mut r: usize) -> T::S {
        ...
        self.update_downward(l);
        self.update_downward(r - 1);
...
#[cfg(test)]
mod tests {
    use crate::{SegmentTree, SegmentTreeSpec};
    ...
    #[test]
    fn test_eager() {
        let mut segment_tree: SegmentTree<AssignMax> =
            SegmentTree::new(vec![2, 3, 8, 4, 0, 1, 3, 9]);
    ...
    fn test_lazy_incomplete() {
        let mut segment_tree: SegmentTree<AssignMax> =
            SegmentTree::new(vec![2, 3, 8, 4, 0, 1, 3, 9, 10]);
        segment_tree.update(2, 7, &12);
        assert_eq!(20, segment_tree.query(1, 3));
        assert_eq!(15, segment_tree.query(4, 7));
